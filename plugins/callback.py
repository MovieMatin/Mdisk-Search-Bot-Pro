...
async def give_access_handler(c: Client, query: CallbackQuery):
    try:
        ...
        update = await db.update_group(
            str(group_id).replace("-100", ""),
            {"has_access": True, "last_verified": datetime.now()},
        )

        txt = await query.edit_message_text(
            f"Group [{group_id}] has access",
        )

        return await c.send_message(
            from_user,
            f"Your group has been authenticated by Admin. Now you can add your own db channel for {Config.VERIFIED_TIME} days.",
        )
    ...
    try:
        ...
        try:
            await db.update_group(
                str(group_id).replace("-100", ""), {"db_channel": int(db_channel)}
            )
        except Exception as e:
            print(e)

        await query.edit_message_text(
            "DB Channel Verified. Make sure you join the channel"
        )
        return await c.send_message(
            from_user,
            f"Your channel {db_channel} has been verified. Bot will search posts from your channel only in your group",
        )
    except Exception as e:
        print(e)
...
async def dbdeny_access_handler(c: Client, query: CallbackQuery):
    ...
    return await c.send_message(
        from_user,
        f"Your request for channel [`{db_channel}`] has been rejected by Admin",
    )
...
async def deny_access_handler(c: Client, query: CallbackQuery):
    ...
    return await c.send_message(
        from_user, "Your request has been rejected by Admin to add your own db channel"
    )
            ...
            [
                [
                    InlineKeyboardButton(
                        "Allow",
                        callback_data=f"give_access#{query.message.chat.id}#{query.from_user.id}",
                    ),
                    InlineKeyboardButton(
                        "Deny",
                        callback_data=f"deny_access#{query.message.chat.id}#{query.from_user.id}",
                    ),
                ],
                [
...
Deny Access: `/deny_access `{group_id}`""",
        ...
        await query.edit_message_text(
            "Request has been sent to Admin. You will be notified when Admin accepts your request"
        )
